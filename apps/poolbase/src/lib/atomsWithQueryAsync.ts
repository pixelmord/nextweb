...
import { ExtractAtomValue, Getter, WritableAtom, atom } from 'jotai';
...
>(getOptions: (get: Getter) => Promise<QueryObserverOptions<TQueryFnData, TError, TData, TQueryData, TQueryKey>>) {
  ...
  const dataAtom = atom(
    ...
    async (get, set, ...args: ExtractAtomArgs<Awaited<ExtractAtomValue<typeof atomsAtom>>[0]>) => {
  ...
  const statusAtom = atom(
    ...
    async (get, set, ...args: ExtractAtomArgs<Awaited<ExtractAtomValue<typeof atomsAtom>>[1]>) => {
