...
const socialIcons = {
  Twitter: () => <FiTwitter className="w-5 h-5 text-accent-900 " />,
  GitHub: () => <FiGithub className="w-5 h-5 text-accent-900 " />,
  LinkedIn: () => <FiLinkedin className="w-5 h-5 text-accent-900 " />,
};
...
export default async function ResumePage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  return (
    <div className="grid bg-white dark:bg-black grid-cols-1 lg:grid-cols-[2.5fr,1fr] w-full mt-6">
      ...
      <div className="bg-base-300 dark:bg-base-600">
        <div className="p-4 text-center">
          ...
          <H2 styling="h3" className="inline-block px-6">
            {data.basics.name}
          </H2>
        ...
        <div className="p-5 space-y-3 bg-base-500 dark:bg-base-400">
          <a className="flex items-center space-x-2 font-medium " href={`mailto:${data.basics.email}`}>
            <FiMail className="w-5 h-5 text-accent-900 " /> <span className="inline-block">{data.basics.email}</span>
          </a>
          <a className="flex items-center space-x-2 font-medium" href={`tel:${data.basics.phone}`}>
            <FiPhone className="w-5 h-5 text-accent-900 " /> <span className="inline-block">{data.basics.phone}</span>
          </a>
          <a className="flex items-center space-x-2 font-medium" href={`${data.basics.website}`}>
            <FiLink className="w-5 h-5 text-accent-900 " /> <span className="inline-block">{data.basics.website}</span>
          </a>
          <hr />
